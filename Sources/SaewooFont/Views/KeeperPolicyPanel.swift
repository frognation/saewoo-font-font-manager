...
struct KeeperPolicyPanel: View {
    ...
    private var header: some View {
        Button { withAnimation(.easeInOut(duration: 0.15)) { expanded.toggle() } } label: {
            HStack(spacing: 8) {
                ...
                Text("which copy survives, for every group")
    ...
    private var rules: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Rules — higher wins").font(.caption).foregroundStyle(.secondary)
            ...
            Toggle(isOn: Binding(
                ...
                VStack(alignment: .leading, spacing: 1) {
                    Text("Never leave the last copy somewhere unreadable")
                        .font(.caption)
                    Text("Rules out unmounted volumes and cloud placeholders — a path with no bytes behind it")
                        .font(.caption2).foregroundStyle(.secondary)
                }
            }
            .toggleStyle(.checkbox)

            Label("macOS essentials and SIP-protected files are always kept, whatever the rules say",
    ...
    private var outcome: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 6) {
                Text("With these rules").font(.caption).foregroundStyle(.secondary)
                if lib.isPreviewingPolicy {
            ...
            row("Files deleted", "\(p.filesDeleted)")
            row("Reclaimed",
                ByteCountFormatter.string(fromByteCount: p.bytesReclaimed, countStyle: .file))
            if p.protectedSkipped > 0 {
                row("Protected, skipped", "\(p.protectedSkipped)", tint: .secondary)
            }

            if p.onlyCopyOffline > 0 {
                VStack(alignment: .leading, spacing: 4) {
                    if p.onlyCopyOffline > 0 {
                        Label("\(p.onlyCopyOffline) groups would keep their only copy somewhere **offline right now**",
                              systemImage: "exclamationmark.octagon.fill")
                            .font(.caption).foregroundStyle(.red)
                    }
                    if p.onlyCopyOffline > 0 {
                        Text("An unreadable copy cannot be registered. Mount the volume or finish syncing, then scan again.")
                            .font(.caption2).foregroundStyle(.secondary)
                    }
                }
                .padding(8)
            ...
            if p.onlyCopyInCloud > 0 {
                row("Kept in cloud (replicated)", "\(p.onlyCopyInCloud)", tint: .secondary, small: true)
            }
            if p.onlyCopyOnSingleDisk > 0 {
                row("Kept on this disk only", "\(p.onlyCopyOnSingleDisk)", tint: .secondary, small: true)
            }

            if !p.deletionsByLocation.isEmpty {
                Text("Deleted from").font(.caption2).foregroundStyle(.secondary).padding(.top, 2)
                ForEach(p.deletionsByLocation.sorted { $0.value > $1.value }, id: \.key) { k, n in
                    row(k, "\(n)", small: true)
                }
            }
            if !p.keepersByLocation.isEmpty {
                Text("Survivors in").font(.caption2).foregroundStyle(.secondary).padding(.top, 2)
