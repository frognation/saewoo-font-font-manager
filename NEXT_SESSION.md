...
Saewoo Font macOS 폰트매니저 작업 이어가자. 모든 컨텍스트는 두 문서에 있음:
  ...
  브랜치:     main  (feature branch는 필요할 때만)
...
시작 전에 반드시 다음 세 파일을 **이 순서대로** 읽어서 전체 맥락 파악해줘:
  ...
  3. Sources/SaewooFont/ — 실제 코드. 특히:
                           Services/FontLibrary.swift (중앙 코디네이터)
                           Services/UFOExporter.swift (Fork 구현)
                           Services/SystemFontGuard.swift (중복 제거 안전장치)
                           Services/DocumentFontImporter.swift (Document Fonts)
                           Services/FontSourceWatcher.swift (FSEvents 자동 재스캔)
...
detail popover, Document Fonts (Figma/Illustrator/manual import), FSEvents
자동 재스캔. 키보드 입력/성능 모두 잡힘.

## HANDOFF.md "Queued" 에 적힌 우선순위 (이걸 이어서 하면 됨)

  1. Duplicates 툴 — 삭제 전 백업 시스템 (복구 가능한 안전한 저장소)
       → ~/Library/Application Support/SaewooFont/DuplicateBackups/{timestamp}/
         + manifest.json (원본 경로 기록) + "Restore last delete" 버튼
  2. Duplicates 툴 — 리스트 뷰 필터 (경로 / 이름 / 용량 정렬)
  3. Duplicates 툴 — source별 delete-lock (절대 지우지 않을 소스 지정)
  4. Google Fonts — WOFF2→TTF 폴백 (CTFontManagerRegisterFontURLs 실패 시)
  5. Google Fonts — variable-axis 감지 (metadata axes 필드 활용)
  6. Adobe Fonts — AdobeFnt*.lst 파싱으로 메타데이터 보강

위 중에서 [번호]번을 구현해줘. 설계 → 구현 → 빌드 검증 → 커밋/푸시 순.
작업 전에는 HANDOFF.md Session 4 섹션 꼭 확인해서 기존 패턴
(SystemFontGuard, derivedVersion 캐싱, Task.detached 캡처 방식,
NewCollectionPrompt sheet 패턴, sourceChangeDebounce 방식)을 따라줘.

## 작업 규칙

- 현재 기준 브랜치는 main. 큰 실험은 feature 브랜치를 따도 되지만,
  낡은 work/compB 지침은 무시.
- 커밋마다 `swift build` 통과 확인 후 푸시
- 커밋 메시지는 지금까지 스타일 그대로 (feat / fix / chore / 이유 본문 + Co-Authored-By)
- 다른 컴에서도 같은 리포 작업 중일 수 있음 → 시작할 때 `git pull --ff-only`,
  끝날 때 반드시 push. force push 금지.
- 내 폰트 라이브러리는 45k+ faces라서 성능 민감 — 새 기능도 캐싱/debounce 고려.
```
...
- **`main`**: 현재 활성 브랜치. 모든 세션 작업이 여기 병합됨.
- **feature branches**: 큰 실험이나 충돌 위험이 있는 작업에서만 사용.
- 다른 컴으로 옮길 때:
  ```bash
